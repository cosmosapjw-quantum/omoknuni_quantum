            ...
            for i, req in enumerate(requests):
                try:
                    ...
                    if state.shape != expected_shape:
                        self.logger.warning(f"Invalid state shape at index {i}: {state.shape}, expected {expected_shape}")
                        # Send error response for invalid state
                        response = EvaluationResponse(
                            request_id=req.request_id,
                            policy=torch.zeros(225),  # Default policy for 15x15 board
                            value=torch.tensor(0.0),
                            error=f"Invalid state shape: {state.shape}"
                        )
                ...
                try:
                    ...
                    for i in indices:
                        ...
                        response = EvaluationResponse(
                            ...
                            value=values_numpy[i],     # Already converted  
                            worker_id=worker_id
                        )
