...
class Evaluator(ABC):
    ...
    def _update_stats(self, batch_size: int, eval_time: float, is_batch: bool = False):
        """Update evaluation statistics"""
        self.stats['evaluations'] += batch_size
        if is_batch or batch_size > 1:
...
class AlphaZeroEvaluator(Evaluator):
    ...
    def evaluate(
        ...
        return policy, value.item() if hasattr(value, 'item') else float(value)
