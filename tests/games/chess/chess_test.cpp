...
TEST_F(ChessTest, TensorRepresentation) {
    ...
    // Should have exactly 20 planes (Option C format)
    EXPECT_EQ(enhancedTensor.size(), 20);
    
    // Verify channels are as expected (Option C format):
    // 0: Current board state (piece encoding)
    // 1: Current player
    // 2-17: Move history (8 pairs)
    // 18-19: Attack and defense planes
